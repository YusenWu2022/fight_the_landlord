...
double extra_point(const int num, const int i)
{
	...
	if (myPosition == 0)
	{
		...
		//test whether pushes the bot to push cards
		if (cardRemaining[1] <= 4)
			extra += cardRemaining[1] / 5;
		if (cardRemaining[2] <= 4)
			extra += cardRemaining[2] / 5;
	}
	else
	{
...
{
	//需要修改，使得倒数第二步抢牌权
	if (Cardnum <= 0 || kth == 6) //可以写一个时间监视器，加入搜索终结条件
	{
		//if (Cardnum <= 0 && depth == 1) //两手牌，可以优先抢
		//score -= 1;
		score += 0.2 * HowManyTimes;
		if (maxscore > score)
		{
			maxscore = score;
...
CardCombo findBestValid()
{
	...
	if (lastValidCombo.comboType != CardComboType::PASS)
	{
		...
		if (lastValidCombo.comboType == CardComboType::SINGLE)
		{
			...
			if (Cardnum <= 8 && judge_last_two())
			{
				...
				for (int i = MAX_LEVEL - 1; i >= lastValidCombo.comboLevel + 1; i--)
				{
					if (Curmycard[i] != 0)
					{
						bestcard.clear();
			...
			for (int i = lastValidCombo.comboLevel + 1; i < MAX_LEVEL; ++i)
			{
				...
				if (Curmycard[i] != 0)
				{
					if ((double)(end_time - begin_time) / CLOCKS_PER_SEC > 0.95)
					{
						bestcard.clear();
						pushcard(i, 1);
						value = -1e9;
		...
		else if (lastValidCombo.comboType == CardComboType::PAIR)
		{
			...
			if (Cardnum <= 8 && judge_last_two())
			{
				...
				for (int i = MAX_LEVEL - 3; i >= lastValidCombo.comboLevel + 1; --i)
				{
					if (Curmycard[i] == 2)
					{
						bestcard.clear();
			...
			for (int i = lastValidCombo.comboLevel + 1; i < MAX_LEVEL - 2; ++i)
			{
				...
				{
					end_time = clock();
					if ((double)(end_time - begin_time) / CLOCKS_PER_SEC > 0.9)
					{
						bestcard.clear();
						pushcard(i, 2);
						value = -1e9;
				 ...
				 lastValidCombo.comboType == CardComboType::PLANE || lastValidCombo.comboType == CardComboType::PLANE2 || lastValidCombo.comboType == CardComboType::PLANE1)
		{
			...
			for (int i = lastValidCombo.comboLevel + 2 - s; i <= MAX_LEVEL - 2 - s;)
			{
				...
				int j = 0;
				while (i + j < MAX_LEVEL - 2 && Curmycard[i + j] == 3) //这里不考虑拆炸弹！苏维埃信仰）
				{
					j++;
				}
				if (j >= s) //找到了三条
				{
					for (int t = 0; t <= j - s; ++t)
					{
						evaluate_prepare();
						int sc[4], single = 0;
						for (int k = i + t; k < i + s + t; ++k) //s是三条数目，因为可以相似处理，路径都差不多
						{
							Curmycard[k] -= 3;
							Cardnum -= 3;
						}
						...
						end_time = clock();
						if ((double)(end_time - begin_time) / CLOCKS_PER_SEC > 0.5)
						{
	...
	else
	{
		...
		else
		{
			...
			for (int i = 0; i < 8; i++)
				if ((Curmycard[i] >= 2 && Curmycard[i] != 4) && (Curmycard[i + 1] >= 2 && Curmycard[i + 1] != 4) && (Curmycard[i + 2] >= 2 && Curmycard[i + 2] != 4))
				{
					int after = 0;
					for (;;)
					{
						if (i + 2 + after >= 11)
							break;
						after = after + 1;
						if (Curmycard[i + 2 + after] == 2 || Curmycard[i + 2 + after] == 3)
							;
						else
						{
							after = after - 1;
			...
			for (int i = 0; i < 8; i++)																																																										  //单顺,到8910jq
				if ((Curmycard[i] >= 1 && Curmycard[i] != 4) && (Curmycard[i + 1] >= 1 && Curmycard[i + 1] != 4) && (Curmycard[i + 2] >= 1 && Curmycard[i + 2] != 4) && (Curmycard[i + 3] >= 1 && Curmycard[i + 3] != 4) && (Curmycard[i + 4] >= 1 && Curmycard[i + 4] != 4)) //单顺，在8910jq结束
				{
					int after = 0;
					for (;; after++)
					{
						if (i + 4 + after < 11 && (Curmycard[i + 4 + after] == 1 || Curmycard[i + 4 + after] == 2))
							;
						else
							break;
					}
					bestcard.clear();
					for (int j = i; j <= i + 4 + after; j++)
					{
						pushcard(j, 1);
					}
					goto flag;
...
//最新日志：双顺bug已经解决，但是三带一会卡bug,三带二没有问题（通过修正了地主的牌数改正了越界问题
//三带一bug：超时。。。。。就是超时，但是三带二却没有问题。。。属实魔幻